...
class TestAbsoluteDates:
    def test_full_month_d_name_year(self) -> None:
    ...
    def test_us_dash_format(self) -> None:
        assert parse("12-01-2025") == date(2025, 12, 1)
...
class TestRelativeToDate:
    def test_before_absolute(self) -> None:
        assert parse("5 days before December 1st, 2025") == date(2025, 11, 26)

    def test_after_absolute(self) -> None:
    ...
    def test_weeks_before(self) -> None:
        assert parse("2 weeks before December 1st, 2025") == date(2025, 11, 17)

    def test_relative_to_yesterday(self) -> None:
        assert parse("3 days after yesterday", date(2025, 6, 10)) == date(2025, 6, 12)

    def test_relative_to_tomorrow(self) -> None:
        assert parse("5 days before tomorrow", date(2025, 6, 10)) == date(2025, 6, 6)

    def test_relative_to_today(self) -> None:
        assert parse("1 week before today", date(2025, 6, 10)) == date(2025, 6, 3)


class TestCompoundRelative:
    def test_years_and_months_after(self) -> None:
        assert parse("1 year and 2 months after yesterday", date(2025, 6, 1)) == date(
            2026, 7, 31
        )

    def test_years_months_days_before(self) -> None:
        assert parse("1 year 2 months 3 days before 2025-01-01") == date(2023, 10, 29)

    def test_weeks_and_days_after(self) -> None:
        assert parse("2 weeks and 3 days after January 1st, 2025") == date(2025, 1, 18)
...
class TestEdgeCases:
    def test_ordinal_numbers_in_between(self) -> None:
        assert parse("5th day after January 1st, 2025") == date(2025, 1, 6)

    def test_no_space_after_comma(self) -> None:
        assert parse("December 1st,2025") == date(2025, 12, 1)

    def test_lowercase_month(self) -> None:
        assert parse("december 1st, 2025") == date(2025, 12, 1)

    def test_lowercase_relative(self) -> None:
        assert parse("in 3 days", date(2025, 6, 1)) == date(2025, 6, 4)
    ...
    def test_midnight_does_not_matter(self) -> None:
